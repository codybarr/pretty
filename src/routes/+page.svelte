...
let accent = $derived(
	`color-mix(in srgb, ${hex} ${mode === "light" ? "11%" : "24%"}, ${mode === "light" ? "white" : "#171717"})`,
);
...
const segment =
	"relative z-10 cursor-pointer whitespace-nowrap rounded-xs px-sm py-sm text-sm text-muted-foreground ark-checked:text-foreground data-focus-visible:outline-2 data-focus-visible:outline-primary";
const segmentRoot =
	"relative isolate flex rounded-sm border border-border bg-muted p-xs";
const segmentIndicator =
	"absolute segment-indicator z-0 rounded-xs border border-border bg-surface shadow-sm transition-all duration-150 motion-reduce:transition-none";
const field = "grid gap-xs text-xs font-medium text-muted-foreground";
const input =
	"w-full rounded-xs border border-border bg-background px-sm py-sm font-sans text-sm tracking-normal text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/20";
</script>
...
<div class="mx-auto max-w-shell px-xl text-foreground max-tablet:px-lg" data-design="slick">
	...
	<main>
		<section class="max-w-intro pt-2xl pb-2xl min-wide:pt-3xl max-phone:pt-2xl max-phone:pb-xl">
			<div class="flex items-center gap-sm font-mono text-xs text-muted-foreground"><span class="size-xs bg-primary"></span><span>SLICK / THEME WORKSHOP</span><span class="w-lg border-t border-border"></span><span>01 / 01</span></div>
			<h1 class="mt-md mb-sm text-display leading-tight font-bold text-foreground max-phone:text-mobile-display">Make the details <span class="text-primary">yours.</span></h1>
			<p class="mt-md max-w-copy text-base leading-relaxed text-muted-foreground">A clean foundation for your next interface. Pick a color, adjust the feel, and watch the system come together.</p>
		</section>
		...
		<div class="grid workspace-columns items-start gap-2xl pb-4xl min-wide:gap-4xl max-tablet:gap-lg max-phone:flex max-phone:flex-col max-phone:gap-xl max-phone:pb-2xl">
			<aside class="w-full rounded-md border border-border bg-surface p-lg pb-md shadow-sm max-tablet:px-md max-tablet:py-md" aria-label="Theme controls">
			...
			<section class="min-w-0 max-phone:w-full" id="preview" aria-label="Live theme preview">
				<div class="flex items-end justify-between pb-md pt-xs"><div><span class={kicker}>THE THEME, IN CONTEXT</span><h2 class="mt-sm text-section font-semibold max-phone:text-xl">From color to composition</h2></div><div class="flex items-center gap-sm pb-xs font-mono text-xs text-muted-foreground"><span class="size-xs rounded-full bg-primary"></span> LIVE</div></div>
				<div class="min-h-preview-height overflow-hidden rounded-md border border-border bg-background text-foreground shadow-sm transition-colors duration-200 max-phone:min-h-0 motion-reduce:transition-none" data-theme={mode} data-corners={corners} style:--color-primary={hex} style:--color-primary-foreground={foreground} style:--color-accent={accent} style:--color-accent-foreground={accentForeground}>
